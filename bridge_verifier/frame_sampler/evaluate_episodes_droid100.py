#!/usr/bin/env python3
"""
Evaluate VLA-SigLIP2 on DROID-100 dataset (100 episodes).

This script:
1. Loads all 100 episodes from DROID-100 dataset
...
Usage:
    # Single GPU
    python evaluate_episodes_droid100.py \
        --checkpoint /path/to/checkpoint.pt \
        --data_dir /root/data/droid_100 \
        --seed 42
    ...
    python evaluate_episodes_droid100.py \
        ...
        --data_dir /root/data/droid_100 \
        --num_gpus 8 \
        --save_videos_dir ./episode_videos_droid100 \
        --save_top_bottom 5 \
        --seed 42
"""
...
def load_episodes_from_droid100(
    data_dir: str,
    ...
    """
    Load all episodes from DROID-100 RLDS dataset.
    ...
    print(f"Loading DROID-100 dataset from {data_dir}...")
    # Use droid_100 dataset with version 1.0.0
    ...
    # No filtering - load all episodes from DROID-100
    # (The dataset already contains curated 100 episodes)
    
    episodes = []
    
    print(f"Processing all episodes from DROID-100...")
    for traj in tqdm(dataset, desc="Loading episodes"):
    ...
    print(f"Loaded {len(episodes)} episodes from DROID-100")
    return episodes
...
def save_episodes_as_videos(
    ...
    for rank, orig_idx in enumerate(sorted_indices):
        ...
        avg_score = details[orig_idx]['avg_score']
...
def evaluate_episodes(
    ...
    Args:
        ...
        episodes: List of episode dicts
        max_frames_per_episode: Optional limit on frames per episode (for speed)
    ...
    for ep_idx, episode in enumerate(tqdm(episodes, desc="Evaluating episodes")):
        ...
        if max_frames_per_episode and len(frames) > max_frames_per_episode:
            ...
            indices = np.linspace(0, len(frames) - 1, max_frames_per_episode, dtype=int)
...
def evaluate_episodes_multi_gpu(
    ...
    Args:
        ...
        max_frames_per_episode: Max frames per episode
...
def print_results(results: dict):
    ...
    print("\n" + "=" * 70)
    print("DROID-100 EPISODE EVALUATION RESULTS")
    print("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate VLA-SigLIP2 model on DROID-100 dataset (all 100 episodes)"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default="/root/data/droid_100",
        help="Parent directory of droid_100/ TFDS folder",
    )
    parser.add_argument(
        "--max_frames_per_episode",
        type=int,
    ...
    # Load all episodes from DROID-100
    episodes = load_episodes_from_droid100(
    ...
    print(f"\nLoaded {len(episodes)} episodes from DROID-100 dataset")
